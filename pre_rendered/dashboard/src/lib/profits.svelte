...
<script>
	...
	async function fetchProfitSummary() {
		...
		transactionsError = '';
		try {
			const response = await axios.get('http://localhost:3000/api/data/profits/summary');
	...
	async function fetchProfitByItem() {
		...
		byItemError = '';
		try {
			const response = await axios.get('http://localhost:3000/api/data/profits/byitem');
                    ...
                    <tbody>
                        {#each profitByItem as item (item.itemcode)}
                            <tr>
                                ...
                                <td class="profit-value">{formatCurrency(item.total_profit_for_item)}</td>
                            </tr>
                        {/each}
                    </tbody>
                </table>
                    ...
                    <tbody>
                        {#each profitTransactions as tx (tx.sale_id)}
                            <tr class="item-row">
                                ...
                                <td class="profit-value">{formatCurrency(tx.total_profit_for_transaction)}</td>
                            </tr>
                        {/each}
                    </tbody>
                </table>
