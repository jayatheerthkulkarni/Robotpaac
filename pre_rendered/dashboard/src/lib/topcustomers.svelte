<script>
	...
	async function fetchData() {
		...
		errorMessage = '';
		try {
			const response = await axios.get(`http://localhost:3000/api/data/top/customers?limit=${limit}`);
	...
	function getMetricLabel() {
		...
		if (displayMetric === 'total_quantity_purchased') return 'Total Quantity Purchased';
		...
		if (chartCanvas && topCustomers.length > 0) {
			...
			chartInstance = new Chart(chartCanvas, {
				...
				options: {
					...
					scales: {
						x: { beginAtZero: true, title: { display: true, text: getMetricLabel() }},
						y: { title: { display: true, text: 'Customer' } }
					},
					plugins: {
						legend: { display: false },
						title: { display: true, text: `Top ${limit} Customers by ${getMetricLabel()}` }
					}
				}
			});
	...
	<div class="header">
		...
		<div class="controls">
			...
			<select id="customer-metric-select" bind:value={displayMetric}>
				...
				<option value="total_quantity_purchased">Total Quantity Purchased</option>
				...
				<tbody>
					{#each topCustomers as customer (customer.custcode)}
						<tr>
							...
							<td class:profit-value={customer.total_profit_from_customer > 0} class:loss-value={customer.total_profit_from_customer < 0}>
