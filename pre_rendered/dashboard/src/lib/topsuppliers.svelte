<script>
	...
	async function fetchData() {
		...
		errorMessage = '';
		try {
			const response = await axios.get(`http://localhost:3000/api/data/top/suppliers?limit=${limit}`);
	...
	function getMetricLabel() {
		...
		if (displayMetric === 'total_quantity_supplied') return 'Total Quantity Supplied';
		if (displayMetric === 'total_batches_supplied') return 'Total Batches Supplied';
		return 'Value';
	}
			...
			chartInstance = new Chart(chartCanvas, {
				...
				options: {
					...
					plugins: {
						...
						title: { display: true, text: `Top ${limit} Suppliers by ${getMetricLabel()}` }
					}
				}
			});
