<script>
  ...
  async function fetchBatchSummary() {
    ...
    try {
      const response = await axios.get(
        'http://localhost:3000/api/data/batches/summary',
      );
  ...
  async function fetchBatchDetails(batchcode) {
    ...
    try {
      const response = await axios.get(
        `http://localhost:3000/api/data/batch/details/${batchcode}`,
      );
        ...
        <tbody>
          {#each batches as batch (batch.batchcode)}
            ...
            >
              ...
              <td
                >{batch.current_stock === null ||
                batch.current_stock === undefined
                  ? 'N/A (Not in Inv.)'
                  : batch.current_stock}</td
              >
  ...
  >
    ...
    >
      ...
      {:else if selectedBatchDetails}
        <h3 class="modal-title" id="modal-title-id">
          Batch Details: {selectedBatchDetails.batchcode}
        </h3>
        <div class="details-grid">
          <div class="detail-item">
            <span class="detail-label">Item Name:</span>
            {selectedBatchDetails.itemname}
          </div>
          <div class="detail-item">
            <span class="detail-label">Item Code:</span>
            {selectedBatchDetails.itemcode}
          </div>

          <div class="detail-item">
            <span class="detail-label">Supplier:</span>
            {selectedBatchDetails.suppname} ({selectedBatchDetails.suppcode})
          </div>
          <div class="detail-item">
            <span class="detail-label">Supplier Contact:</span>
            {selectedBatchDetails.suppcontact}
          </div>

          <div class="detail-item">
            <span class="detail-label">Date Purchased:</span>
            {formatDate(selectedBatchDetails.datepurchase)}
          </div>
          <div class="detail-item">
            <span class="detail-label">Expiry Date:</span>
            {formatDate(selectedBatchDetails.expiry)}
          </div>

          <div class="detail-item">
            <span class="detail-label">Requested Qty:</span>
            {selectedBatchDetails.requestedqty}
          </div>
          <div class="detail-item">
            <span class="detail-label">Received Qty:</span>
            {selectedBatchDetails.receivedqty}
          </div>

          <div class="detail-item">
            <span class="detail-label">Current Stock (Inventory):</span>
            {selectedBatchDetails.current_stock_in_inventory === null ||
            selectedBatchDetails.current_stock_in_inventory === undefined
              ? 'N/A'
              : selectedBatchDetails.current_stock_in_inventory}
          </div>
          <div class="detail-item">
            <span class="detail-label">Total Sold from Batch:</span>
            {selectedBatchDetails.total_sold_from_batch || 0}
          </div>
          ...
          {#if selectedBatchDetails.itemused || selectedBatchDetails.itemdesc1 || selectedBatchDetails.itemdesc2 || selectedBatchDetails.itemdesc3}
            ...
            {#if selectedBatchDetails.itemused}
              <div class="detail-item full-width">
                <span class="detail-label">Item Used For:</span>
                {selectedBatchDetails.itemused}
              </div>
            {/if}
            {#if selectedBatchDetails.itemdesc1}
              <div class="detail-item full-width">
                <span class="detail-label">Description 1:</span>
                {selectedBatchDetails.itemdesc1}
              </div>
            {/if}
            {#if selectedBatchDetails.itemdesc2}
              <div class="detail-item full-width">
                <span class="detail-label">Description 2:</span>
                {selectedBatchDetails.itemdesc2}
              </div>
            {/if}
            {#if selectedBatchDetails.itemdesc3}
              <div class="detail-item full-width">
                <span class="detail-label">Description 3:</span>
                {selectedBatchDetails.itemdesc3}
              </div>
            {/if}
          {/if}
        </div>
