<script>
	...
	async function fetchData() {
		...
		errorMessage = '';
		try {
			const response = await axios.get('http://localhost:3000/api/data/total/items/sold');
			const data = response.data;

			grandTotalSold = data.grandTotal || 0;
			
			const timeSeriesChartData = data.timeSeries.map(d => ({
				x: new Date(d.dateout),
				y: d.total_sold_on_date
			}));
...
<div class="container">
	<div class="header">
		<p class="title-text">Total Items Sold Analysis</p>
	</div>
	...
	{:else}
		<div class="total-summary-card">
			<p class="total-label">Grand Total Items Dispatched</p>
			<p class="total-value">{grandTotalSold.toLocaleString()}</p>
		</div>
		...
		<div class="chart-wrapper">
			...
			{:else}
				<p class="no-data-message">No dispatch data available to display chart.</p>
			{/if}
		</div>
	{/if}
</div>
